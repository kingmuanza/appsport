...
<html>
                    ...
                    buttons: [
                        {
                            ...
                            className: 'nouveau',
                            action : function(){
                                metroDialog.open('#dialog-discipline');
                            }
                        },
                        {
                            extend: 'excel',
    ...
    <body>
        <div class="grid condensed" style="margin: 0px;">
            ...
            <div id="affichageTableau" class="row cells4" style="padding-right: 20px; padding-top: 20px;">
                <table id="historique" class="dataTable" style="margin-bottom: 10px;">
                    ...
                    <tbody>
                        <c:forEach items="${utilisateurs}" var="d" varStatus="loop">
                            <tr>
                                <td>${loop.count}</td>
                                <td>
                                    <span style="font-weight: 600; cursor: pointer" onclick="metroDialog.open('#dialog-discipline${d.idutilisateur}');">
                                        ${d.noms}
                                    </span>
                                </td>
                                <td>${d.prenoms}</td>
                                <td>${d.login}</td>
                                <td>${d.utilisateurProfil.libelle}</td>
                                <td>${d.organisation.libelle}</td>
                            </tr>
                        </c:forEach>
        ...
        <div data-role="dialog" id="dialog-discipline" class="bg-white padding20"
             data-close-button="true"
             data-windows-style="false"
             data-href="UtilisateurServlet?temps=${temps}"
             data-overlay="true"
             data-color="fg-dark"
             data-width="600"
        ...
        <c:forEach items="${utilisateurs}" var="d">
            <div data-role="dialog" id="dialog-discipline${d.idutilisateur}" class="bg-white padding20"
                 data-close-button="true"
                 data-windows-style="false"
                 data-href="UtilisateurServlet?id=${d.idutilisateur}&temps=${temps}"
                 data-overlay="true"
                 data-color="fg-dark"
                 data-width="600"
                 data-overlay-color="overlay-bleu"
