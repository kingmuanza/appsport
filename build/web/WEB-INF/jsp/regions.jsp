...
<html>
                    ...
                    buttons: [
                        {
                            ...
                            className: 'nouveau',
                            action: function () {
                                metroDialog.open('#dialog-region');
                            }
                        },
                        {
                            extend: 'excel',
    ...
    <body>
        <div class="grid condensed" style="margin: 0px;">
            ...
            <div id="affichageTableau" class="row cells4" style="padding-right: 20px; padding-top: 20px;">
                <table id="historique" class="dataTable" style="margin-bottom: 10px;">
                    ...
                    <tbody>
                        <c:forEach items="${regions}" var="d" varStatus="loop">
                            <tr onclick="metroDialog.open('#dialog-region${d.idregion}');">
                                <td>${loop.count}</td>
                                <td>
                                ...
                                <td>${d.libelle}</td>
                                <td>
                                    <c:forEach items="${d.organisations}" var="o">
                                        ${o.libelle}
                                    </c:forEach>
                                </td>
                                <td>
                                    <c:forEach items="${d.organisations}" var="o">
                                        <c:forEach items="${o.utilisateurs}" var="ut">
                                            <b class="fg-bleu">${ut.login}</b><br>
                                        </c:forEach>
                                    </c:forEach>
                                </td>
                            </tr>
        ...
        <div data-role="dialog" id="dialog-region" class="bg-white padding20"
             data-close-button="true"
             data-windows-style="false"
             data-href="RegionServlet?temps=${temps}"
             data-overlay="true"
             data-color="fg-dark"
        ...
        <c:forEach items="${regions}" var="d">
            <div data-role="dialog" id="dialog-region${d.idregion}" class="bg-white padding20"
                 data-close-button="true"
                 data-windows-style="false"
                 data-href="RegionServlet?id=${d.idregion}&temps=${temps}"
                 data-overlay="true"
                 data-color="fg-dark"
                 data-width="600"
                 data-overlay-color="overlay-bleu"
